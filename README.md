...
Now our `pipelines_network` is an API to interract with our pipelines network and it has only two options: ```ts public connectToPort(port: number, entity: any)` and `public dispatchToAll(message: string, data?: any)```
